...
document.addEventListener('DOMContentLoaded', () => {
    function getDeviceFingerprint() { const fingerprint = [navigator.userAgent, navigator.language, screen.width, screen.height, new Date().getTimezoneOffset(), navigator.hardwareConcurrency,].join('|'); let hash = 0; for (let i = 0; i < fingerprint.length; i++) { const char = fingerprint.charCodeAt(i); hash = ((hash << 5) - hash) + char; hash = hash & hash; } return hash.toString(); }
    ...
    if (loggedInRollNo) { showView('dashboard'); updateStudentInfo(loggedInRollNo); } else { showView('login'); }
    ...
    loginBtn.addEventListener('click', () => { const rollNo = rollInput.value.trim(); if (rollNo && !isNaN(rollNo)) { loggedInRollNo = rollNo; sessionStorage.setItem('loggedInStudentRoll', rollNo); updateStudentInfo(rollNo); loginError.textContent = ''; showView('dashboard'); } else { loginError.textContent = 'Please enter a valid Roll Number.'; } });
    ...
    async function handleQrCodeData(data) { const parts = data.split('|'); const [sessionId, timestamp] = parts; if (parts.length !== 2 || !sessionId || !timestamp) { showResult('Invalid QR code format.', 'error', 'InAppScan'); return; } const codeAgeMs = Date.now() - parseInt(timestamp); if (codeAgeMs > 20000) { showResult('Expired QR code. Please scan the new one.', 'error', 'InAppScan'); return; } await markAttendance(parseInt(sessionId), 'InAppScan'); }
    ...
    function showResult(message, type, method) { let resultEl; if (method === 'InAppScan') { showView('dashboard'); resultEl = attendanceResultEl; } else { resultEl = manualCodeResult; } resultEl.textContent = message; resultEl.className = type === 'success' ? 'result-success' : 'result-error'; }
});
