...
//     // --- UTILITY: Check URL for scan data as soon as the page loads ---
//     function checkForScanData() {
//         const urlParams = new URLSearchParams(window.location.search);
//         const scanData = urlParams.get('scan');
//         if (scanData) {
//             try {
//                 const decodedData = atob(scanData); // atob() decodes the data
//                 // Store the valid, pending data to be used after login
//                 sessionStorage.setItem('pendingScanData', decodedData);
//                 // Clean the URL to prevent re-use on refresh
...
//         }
//     }
//     checkForScanData(); // Run the check immediately
...
//                 handleQrCodeData(pendingData, true); // Pass true to indicate it's from a URL scan
//             } else {
...
//         rollInput.value = '';
//         attendanceResultEl.textContent = ''; // Clear dashboard message on logout
//         showView('login');
//     });
...
//             if (code) {
//                 stopScanner();
//                 handleQrCodeData(code.data, false); // Pass false, it's NOT from a URL scan
//                 return;
//             }
//         }
//         animationFrameId = requestAnimationFrame(tick);
...
//         const { data: activeSessions, error } = await db.from('sessions')
...
//             studentRollNumber >= s.classes.start_roll_no &&
...
//     async function handleQrCodeData(data, fromUrl = false) {
//         const parts = data.split('|');
//         const [sessionId, timestamp] = parts;

//         if (parts.length !== 2 || !sessionId || !timestamp) {
//             showResult('Invalid QR code format.', 'error', fromUrl ? 'UrlScan' : 'InAppScan');
//             return;
//         }

//         const codeAgeMs = Date.now() - parseInt(timestamp);
//         if (codeAgeMs > 20000) { // 20-second window
//             showResult('Expired QR code. Please scan the new one.', 'error', fromUrl ? 'UrlScan' : 'InAppScan');
//             return;
//         }

//         await markAttendance(parseInt(sessionId), fromUrl ? 'UrlScan' : 'InAppScan');
//     }
...
//         switch (method) {
//             case 'UrlScan':
//                 // For a scan from the URL, show the result on the login page itself
//                 resultEl = loginError;
//                 break;
//             case 'InAppScan':
//                 // For a scan from the dashboard, show the result on the dashboard
//                 showView('dashboard');
//                 resultEl = attendanceResultEl;
//                 break;
//             case 'Manual Entry':
//                 // For a manual code entry, show the result on the manual code page
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    function showResult(message, type, method) {
        ...
        switch (method) {
            case 'UrlScan':
                // For a scan from the URL, show the result on the login page itself and don't change the view.
                resultEl = loginError;
                break;
            case 'InAppScan':
                // For a scan from the dashboard, show the result on the dashboard and switch to that view.
                showView('dashboard');
                resultEl = attendanceResultEl;
                break;
            case 'Manual Entry':
